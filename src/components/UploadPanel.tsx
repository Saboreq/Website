...
export function UploadPanel({ currentFolderId, onChanged, user }: UploadPanelProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === 'file') {
        ...
        await createFolder(user, currentFolderId, folderName, visibility);
        setFolderName('');
      }
  ...
  return (
    <aside className="upload-panel" aria-labelledby="upload-heading">
      ...
      <div className="segmented-control segmented-control--compact" aria-label="Create type">
        <button aria-pressed={mode === 'file'} onClick={() => setMode('file')} type="button">Upload file</button>
        <button aria-pressed={mode === 'folder'} onClick={() => setMode('folder')} type="button">New folder</button>
      </div>
      ...
      <form className="upload-form" onSubmit={submit}>
        ...
        <fieldset className="visibility-picker">
          ...
          <label>
            ...
            <span><strong>Public</strong><small>Anyone with access to the site can download it.</small></span>
          </label>
          <label>
            <input checked={visibility === 'private'} name="visibility" onChange={() => setVisibility('private')} type="radio" />
            <span><strong>Private</strong><small>Only you can see and download it.</small></span>
          </label>
        </fieldset>
